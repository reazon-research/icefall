...
def download_reazonspeech(
    ...
    :param dataset_dir: Pathlike, the path of the dir on the local storage
    :param dataset_part: the parts of the dataset to download (e.g. small, medium, or large).
    ...
    logging.info(f"Downloading ReazonSpeech part: {dataset_part}")
    if dataset_dir:
        ds = load_dataset(
            "json", data_files={"train": os.path.join(dataset_dir, dataset_part, f"{dataset_part}.jsonl")}
        )["train"]
        ds = ds.rename_columns({"label": "transcription"})
    else:
        ds = load_dataset(
    ...
    ds = ds.cast_column("audio", Audio(decode=False))  # Hack: don't decode to speedup
    ds = ds.map(
        format_example,
        with_indices=True,
    ...
    ds.to_json(
        corpus_dir / dataset_part / "dataset.jsonl",
        num_proc=num_jobs,
        force_ascii=False,
        indent=None,
        lines=True,
        batch_size=min(10_000, ds.num_rows),
    )
...
def prepare_reazonspeech(
    ...
    :param dataset_part: str, the split name of the dataset
    :param num_jobs: int, number of parallel threads used for 'parse_utterance' calls.
    :return: a Dict whose key is the dataset part, and the value is Dicts with the keys 'audio' and 'supervisions'.
    """
    corpus_dir = Path(corpus_dir) / dataset_part
    assert corpus_dir.is_dir(), f"No such directory: {corpus_dir}"

    # Split the dataset into train, dev, and test
    with open(corpus_dir / "dataset.jsonl", "r", encoding="utf-8") as file:
        with open(corpus_dir / "dev.jsonl", "w") as wf:
            file.seek(0)
            for _ in range(1000):
                wf.write(file.readline())
        with open(corpus_dir / "test.jsonl", "w") as wf:
            file.seek(0)
            for i in range(1100):
                line = file.readline()
                if i < 1000:
                    continue
                wf.write(line)
        with open(corpus_dir / "train.jsonl", "w") as wf:
            file.seek(0)
    ...
    output_dir = Path(output_dir) / dataset_part
    ...
    for part in parts:
        ...
        filename = corpus_dir / f"{part}.jsonl"
        with open(filename, "r", encoding="utf-8") as file:
            # items = json.load(file)
            items = [json.loads(line.strip()) for line in file]
...
if __name__ == "__main__":
    ...
    parser.add_argument("task", type=str, choices=["download", "prepare"])
    parser.add_argument("--target-dir", required=True, help="a.k.a. output_dir")
