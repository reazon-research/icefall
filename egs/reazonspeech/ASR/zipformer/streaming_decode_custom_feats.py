...
def decode_one_chunk(
    ...
    if params.decoding_method == "greedy_search":
        greedy_search(model=model, encoder_out=encoder_out, streams=decode_streams, blank_penalty=params.blank_penalty)
...
def decode_dataset(
    ...
    for num, cut in enumerate(cuts):
        ...
        # # Here in the original code, the features are calculated from the fbank on the fly:
        ...
        # Since we calculated the features before during dataset preparation (in prepare.sh),
        # we want to load them from the cut directly instead.

        # print("Attempting to load cut features...")
        # existing_feature = torch.tensor(cut.load_features(), device=device)
        feature = torch.tensor(cut.load_features(), device=device)
        # print(existing_feature)
        # print(len(existing_feature), len(existing_feature[0]))
        # print(len(existing_feature[0][0]))
        # print(dir(existing_feature))
        # print(feature)
        # assert False
        # print("loaded cut features")
            ...
            for i in sorted(finished_streams, reverse=True):
                decode_results.append(
                    ...
                    (
                        ...
                        list(sp.decode(decode_streams[i].decoding_result()).strip()),
                    )
                )
