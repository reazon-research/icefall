...
log "Dataset: CommonVoice English"
if [ $stage -le 2 ] && [ $stop_stage -ge 2 ]; then
  log "Stage 2: Soft link manifests (including fbank) of CommonVoice English"
  if [ -e ../../commonvoice/ASR/data/en/fbank/.preprocess_complete ]; then
    cd data/manifests
    mkdir -p commonvoice_en
    cd commonvoice_en
    ln -svfr $(realpath ../../../../../commonvoice/ASR/data/en/fbank/cv-en_cuts*) .
    ln -svfr $(realpath ../../../../../commonvoice/ASR/data/en/fbank/feats*) .
    cd ../../..
  else
    log "Abort! Please run ../../commonvoice/ASR/prepare.sh --lang en --stage 0 --stop-stage 3"
    exit 1
  fi
fi
...
if [ $stage -le 5 ] && [ $stop_stage -ge 5 ]; then
  ...
  # Check if lang data from CommonVoice English exists
  if [ ! -d ../../commonvoice/ASR/data/en/lang_bpe_500 ] || [ ! -f ../../commonvoice/ASR/data/en/lang_bpe_500/transcript_words.txt ]; then
    log "Abort! Please ensure ../../commonvoice/ASR/data/en/lang_bpe_500 and transcript_words.txt exist."
    log "Please run ../../commonvoice/ASR/prepare.sh --lang en --stage 0 --stop-stage 5 if you haven't already."
    exit 1
  fi
  ...
  # Combine Japanese char-level text and English transcript
  cat data/lang_char/text ../../commonvoice/ASR/data/en/lang_bpe_500/transcript_words.txt \
    > $lang_dir/text
  ...
  for vocab_size in ${vocab_sizes[@]}; do
    ...
    if [ ! -f $bbpe_dir/text_words_segmentation ]; then
      ...
      cat ../../commonvoice/ASR/data/en/lang_bpe_500/transcript_words.txt \
