...
#                                                       Daniel Povey,
#                                                       Xiaoyu Yang)
# Copyright    2024  Reazon Research      (adaptation for CommonVoice fine-tuning)
#
...
"""
Fine-tuning script for CommonVoice English data using pre-trained multi-ja-en model.

Usage:

export CUDA_VISIBLE_DEVICES="0,1,2,3"

# Fine-tune multi-ja-en model on CommonVoice English:
./zipformer/finetune.py \
  ...
  --use-fp16 1 \
  --do-finetune 1 \
  --finetune-ckpt path/to/multi_ja_en_model.pt \
  --base-lr 0.0045 \
  --exp-dir zipformer/exp_finetune_cv_en \
  --language en \
  --bbpe-model path/to/multi_ja_en/bbpe.model \
  --max-duration 1000

Note: This script expects CommonVoice English data to be re-tokenized with the
same BBPE model used for multi-ja-en training, or it will re-tokenize on-the-fly.
"""
...
def get_parser():
    ...
    parser.add_argument(
        "--exp-dir",
        type=str,
        default="zipformer/exp_finetune_cv",
    ...
    parser.add_argument(
        ...
        help="Path to the BBPE model (must match the pre-trained model's tokenizer)",
    )
    ...
    params = AttributeDict(
        {
            ...
            "log_interval": 50,
            "reset_interval": 200,
            "valid_interval": 3000,  # For CommonVoice, may need adjustment
...
def train_one_epoch(
    ...
    for batch_idx, batch in enumerate(train_dl):
        ...
        # Validation during training (re-enabled to match LibriSpeech finetune behavior)
        # If OOM occurs, consider reducing max_duration or increasing valid_interval
        if batch_idx > 0 and batch_idx % params.valid_interval == 0 and not params.print_diagnostics:
            logging.info("Computing validation loss on CommonVoice dev set")
            ...
            model.train()
            logging.info(
                f"Validation: Epoch {params.cur_epoch}, validation: {valid_info}"
            )
            logging.info(
...
def run(rank, world_size, args):
    ...
    # <blk> is defined in local/prepare_lang_bbpe.py
    ...
    # Initialize CommonVoice data module for English
    commonvoice = CommonVoiceAsrDataModule(args)

    # Get CommonVoice English training cuts
    ...
    def remove_short_and_long_utt(c: Cut):
        ...
        # CommonVoice typically has shorter utterances than LibriSpeech
        # Adjust these thresholds based on your CommonVoice data distribution
    ...
    # CommonVoice has single dev set (not clean/other split)
